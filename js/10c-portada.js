...
/* ================= El alta por pasos (EN PRUEBA, 0.7.118) =================
   ...
   Qué borrar si no se queda: al final del bloque `.crear-pasos` en
   `css/estilos.css`. */
function portadaPorPasos() {
  return document.documentElement.classList.contains("puerta-pasos");
}
...
function portadaPintar(modo) {
  ...
  if (modo === "crear") {
    ...
    dentro =
      `<div class="portada-cab">
       ...
       ${portadaPorPasos() ? "" : portadaLegalHTML("portada-legal-arriba")}
       ...
       ${(() => {
         /* Los cinco campos, siempre los mismos y con los mismos ids. Lo único
            ...
            agrupan y cuál se enseña — ver `portadaPorPasos` más arriba. */
    ...
    if (guardadas) {
      dentro += `<p class="cuentas-tit">${tx("Ya entraste aquí con:")}</p>${guardadas}
                 <div class="portada-o"><span>${agregando ? tx("o entra con otra") : tx("o escribe tu correo")}</span></div>`;
    }
  ...
  cap.querySelectorAll("input").forEach(el => {
    el.addEventListener("keydown", e => {
      ...
      if (portadaPorPasos() && portadaPaso < PORTADA_PASOS) portadaPasoIr(portadaPaso + 1);
  ...
  if (modo === "crear") {
    if (portadaPorPasos()) portadaPasoPintar();
...
function portadaCrearAtras() {
  if (portadaPorPasos() && portadaPaso > 1) { portadaPasoIr(portadaPaso - 1); return; }
  portadaIrA("entrar");
}
...
async function portadaOfrecerGoogle(modo) {
  ...
  hueco.innerHTML =
    `<button class="btn btn-soft btn-block portada-google" onclick="sbEntrarConGoogle()">
     ...
     ${/* Entre el botón y la rayita, y solo en el alta por pasos: ahí la frase
          se lee como la letra pequeña de ESE botón, que es de lo que avisa.
          En el formulario largo va arriba, donde siempre. */
       creando && portadaPorPasos() ? portadaLegalHTML("portada-legal-bajo") : ""}
...
async function entrarConCuentaGuardada(uid) {
  ...
  if (!c) {
    toast(tx("Esa cuenta ya no está guardada en este dispositivo."));
  ...
  if (!enLaPuerta() && syncReady() && sync.dirty) {
    ...
    if (sync.dirty) {
      cargaCerrar();
      toast(tx("No cambié de cuenta, para no dejarte atrás lo que falta por subir. ") +
        (syncError || tx("Revisa la conexión e inténtalo otra vez.")));
      return;
    }
  }
  ...
  if (!viva) {
    ...
    if (enLaPuerta()) { portadaPintar("entrar"); } else { renderSync(); }
    toast(tx("La sesión guardada de ") + (saludo || c.correo) + " ya no vale. Entra con su contraseña.");
    return;
  }
...
async function quitarCuentaGuardada(uid) {
  ...
  if (!await ask(tx("Se quita ") + quien + " de este dispositivo. Su progreso sigue entero en su cuenta; para volver a entrar habrá que escribir la contraseña.", "Quitar")) return;
  cuentaOlvidar(uid);
  if (enLaPuerta()) portadaPintar("entrar"); else renderSync();
  toast(tx("Quitada de este dispositivo"), "calma");
}
...
async function irAAgregarCuenta() {
  ...
  if (syncReady() && sync.dirty) {
    ...
    if (sync.dirty) {
      cargaCerrar();
      toast(tx("Me quedé aquí, para no dejarte atrás lo que falta por subir. ") +
        (syncError || tx("Revisa la conexión e inténtalo otra vez.")));
      return;
    }
  }
  ...
  if (!puesto) {
    cargaCerrar();
    toast(tx("Este navegador no me deja guardar el paso intermedio. Cierra sesión y entra con la otra cuenta."));
    return;
  }
