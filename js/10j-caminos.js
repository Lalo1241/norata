...
function cartaDeCamino(c, trabada) {
  ...
  return `<button type="button" class="cam-carta${trabada ? " trabada" : ""}" style="--c:${col}"
    ...
    <span class="cam-cuerpo">
      ...
      <span class="cam-uno"><b>Empiezas por</b><br>${escapeHtml(uno ? uno.nombre : "")}</span>
      <span class="cam-pie">
...
async function abrirCajon(kind) {
  ...
  document.getElementById("cajon-body").innerHTML =
    `<p class="settings-note" style="text-align:center;margin:22px 0">Trayendo los caminos…</p>`;
  ...
  if (!ok) {
    document.getElementById("cajon-body").innerHTML = `
      <div class="cajon-cab"><h3>No se pudieron traer</h3>
      <p>Hace falta conexión para verlos la primera vez. Después ya se quedan.</p></div>
      <div class="cajon-pie" style="margin-top:16px">
        <button class="btn btn-ghost" onclick="cerrarCajon()">Cerrar</button>
...
function pintarCajon() {
  ...
  document.getElementById("cajon-body").innerHTML = `
    <div class="cajon-cab">
      <h3>${proy ? "Un proyecto nuevo" : "Una rama nueva"}</h3>
      <p>${proy
        ...
        : "Un camino la trae armada: <b>peldaños encadenados</b>, con sus pasos, sus plazos y las misiones que la alimentan. Todos salen de métodos publicados, y lo que entre se edita como cualquier cosa tuya."}</p>
    ...
    <div class="cajon-cuerpo">
      ${cabe ? `<button type="button" class="cam-cero" onclick="cerrarCajon();crearRamaDeCero('${escapeAttr(cajonKind)}')">
        ...
        <span><b>De cero</b>Le pones nombre y ${proy ? "lo llenas" : "la llenas"} tú</span>
      </button>` : ""}
      ...
      ${sug.length ? `<p class="cam-rot">Por lo que elegiste al empezar</p>
        <div class="cam-rejilla">${sug.map(c => cartaDeCamino(c, trabada)).join("")}</div>` : ""}
      ${resto.length ? `<p class="cam-rot">${sug.length ? "Los demás" : "Los caminos"}</p>
    ...
    <div class="cajon-pie">
      <button class="btn btn-ghost" onclick="cerrarCajon()">Cancelar</button>
    </div>`;
}
...
function verCamino(id) {
  ...
  document.getElementById("cajon-body").innerHTML = `
      ...
      <div class="cam-lista">
        ${c.peldanos.slice(0, 5).map(p => `<div>
          ...
          <span><b>${escapeHtml(p.nombre)}</b> · ${p.pide.length
            ...
            : "abierto desde el día uno"}</span></div>`).join("")}
    ...
    <div class="cajon-pie">
      <button class="btn btn-ghost" onclick="pintarCajon()">Atrás</button>
      <button class="btn btn-primary" onclick="ponerCamino('${escapeAttr(c.id)}')">Ponerlo en mi tablero</button>
    </div>`;
...
function ponerCamino(id) {
  ...
  const uno = c.peldanos.find(p => !p.pide.length);
  celebrate("Tu camino ya está puesto",
    `${c.peldanos.length} ${proy ? "encargos" : "peldaños"} en «${rama}». ${
      uno ? "Empiezas por: " + uno.nombre : "El primero ya está abierto."}`,
    col, "map");
}
