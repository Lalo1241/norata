...
function pintarMandoZoom(wrap, b) {
  ...
  if (!mando) {
    ...
    mando.innerHTML = `
      ...
      <button type="button" data-z="ajustar" class="ajustar" aria-label="Encuadrar todo el mapa" title="Encuadrar todo el mapa"><svg viewBox="0 0 24 24">${BM_ICONS.encuadrar}</svg></button>`;
...
function plegarTodasLasRamas(plegar) {
  ...
  save();
  renderTree();
  toast(plegar ? "Todas plegadas" : "Todas desplegadas", "hecho");
}
